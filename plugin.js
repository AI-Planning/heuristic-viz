...
var root, d3, zoom, viewerWidth, viewerHeight, actions;
...
function graphData() {
    ...
    const fluents = getGroundedFluents();
    actions = getGroundedActions();
    fluents.forEach(fluent => {
        data.nodes.push({"id":fluent, "name":fluent, "type":"fluent"});
    });
    Array.from(actions['preconditions'].keys()).forEach(action => {
        data.nodes.push({"id":action, "name":action, "type":"action"});
...
function startHeuristicViz(node) {
    ...
    function highlight(d) {
        console.log("ACTIONS: ", actions['preconditions'].get("d.id"));
...
// Grounds and prints out grounded representation for a planning problem (domain, prob)
function testGrounding(domain, problem) {
	pyodide.runPythonAsync(`
		#print(grounder.ground_state_variables())
	
		print(operators)
		
		#for f in operators[0].precondition.subformulas:
			#print(f, type(f))

		#print(operators[0].effects)
	`);
}

async function go() {
	await initializeGrounding();
	await ground(window.sampleGroundingDomain, window.sampleGroundingProblem);
	// await testGrounding(window.sampleGroundingDomain, window.sampleGroundingProblem);
}
